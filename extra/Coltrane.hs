...
--route (r:rs) req = case (renderStdMethod (method r)==requestMethod req, path r) of 
--  (True, Literal l) ->
--  -- match the route's method and path info with the request's
--      case matchesPath ((DT.split (=='/') (DT.pack l))) (pathInfo req) of
--           -- run the handler and return the ResponseState
--       Just ps  -> do (ps',_) <- MS.liftIO $ parseRequestBody lbsBackEnd req
--                      rs <- runHandler r (ps++convertBSParams ps') req
--                      return $ Just rs 
--            -- check the next route if there wasn't a match
--       Nothing  -> route rs req
--  (True, RegExp  re) ->
--      let sr = rawPathInfo req  
--          (sh:ss) =BS.split '?' $ sr
--          s = if BS.null sh then sr else sh
--      in
--      case matchRegex re (BS.unpack s) of
--        Just strs -> do (ps,_) <- MS.liftIO $ parseRequestBody lbsBackEnd req
--                        rs <- runHandler r ((putRegexParams strs)++convertBSParams ps) req
--                        return $ Just rs
--        Nothing -> route rs req
--  _  -> route rs req
...
--in the request
{--Previous version of matchesPath
matchesPath :: [DT.Text] -> [DT.Text] -> Maybe Params
matchesPath ((r1:rs1)) ((r2:rs2)) = 
  case (DT.head r1) of 
    ':' -> liftM2 (++) (Just [(DT.unpack r1, DT.unpack r2)]) (matchesPath (rs1) (rs2))
    _   -> if r1==r2 then matchesPath (rs1) (rs2) else Nothing
matchesPath r1 r2   = if (filter (/=(DT.pack "")) r1) ==(filter (/=(DT.pack "")) r2) then Just [] else Nothing 
...
-- run the framework on the given port and application
